...
def infer():
    ...
    for fi in tqdm(files):
        X, sr = librosa.load(fi, sr=args.sr, mono=False, offset=0.0, duration=None, dtype=np.float32, res_type='kaiser_fast')
        basename = os.path.splitext(os.path.basename(fi))[0]
        if X.ndim == 1:
            # mono to stereo
            X = np.asarray([X, X])
        X_spec = spec_utils.wave_to_spectrogram(X, args.hop_length, args.n_fft)
        ...
        if output_dir != "":  # modifies output_dir if theres an arg specified
            output_dir = output_dir.rstrip('/') + '/'
            # output_dir_intru = output_dir.rstrip('/') + 'intru/'
            os.makedirs(output_dir, exist_ok=True)
            # os.makedirs(output_dir_intru, exist_ok=True)
        # wave = spec_utils.spectrogram_to_wave(y_spec, hop_length=args.hop_length)
        # sf.write('{}{}_Instruments.wav'.format(output_dir_intru, basename), wave.T, sr)
        wave = spec_utils.spectrogram_to_wave(v_spec, hop_length=args.hop_length)
